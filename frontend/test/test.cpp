...
DEFINE_bool(ycsb_tx, false, "");
DEFINE_bool(ycsb_count_unique_lookup_keys, true, "");
// -------------------------------------------------------------------------------------
using namespace leanstore;
// -------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------
// MyNote:: YCSBKey and Payload are of constant size?
using YCSBKey = u64;
...
{
   gflags::SetUsageMessage("Speed Up Leanstore Frontend");
   ...
   if (FLAGS_recover) {
      // MyNote: retreive information about the Btree datastructure
   ...
   cout << "-------------------------------------------------------------------------------------" << endl;
   cout << "~Fast Transactions" << endl;
   keep_running = true;
   running_threads_counter = 0;
   threads.clear();
   for (u64 t_i = 0; t_i < FLAGS_worker_threads; t_i++) {
      threads.emplace_back([&]() {
         running_threads_counter++;
         while (keep_running) {
            ...
            YCSBPayload result;
            if (FLAGS_ycsb_read_ratio == 100 || utils::RandomGenerator::getRandU64(0, 100) < FLAGS_ycsb_read_ratio) {
               table.trained_lookup(key, result);
            } else {
               YCSBPayload payload;
               utils::RandomGenerator::getRandString(reinterpret_cast<u8*>(&payload), sizeof(YCSBPayload));
               table.update(key, payload);
