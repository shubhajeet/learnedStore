...
size_t HistogramBucketMapper::IndexForValue(const uint64_t value) const
{
   if (value >= maxBucketValue_) {
      ...
      std::map<uint64_t, uint64_t>::const_iterator lowerBound = valueIndexMap_.lower_bound(value);
...
void HistogramStat::Add(uint64_t value)
{
   ...
   buckets_[index].store(buckets_[index].load(std::memory_order_relaxed) + 1, std::memory_order_relaxed);
   ...
   sum_squares_.store(sum_squares_.load(std::memory_order_relaxed) + value * value, std::memory_order_relaxed);
}
...
double HistogramStat::StandardDeviation() const
{
   ...
   if (cur_num == 0)
      return 0;
   double variance = static_cast<double>(cur_sum_squares * cur_num - cur_sum * cur_sum) / static_cast<double>(cur_num * cur_num);
...
std::string HistogramStat::ToString() const
{
   ...
   for (unsigned int b = 0; b < num_buckets_; b++) {
      ...
      if (bucket_value <= 0.0)
         continue;
      cumulative_sum += bucket_value;
      snprintf(buf, sizeof(buf), "%c %7" PRIu64 ", %7" PRIu64 " ] %8" PRIu64 " %7.3f%% %7.3f%% ", (b == 0) ? '[' : '(',
