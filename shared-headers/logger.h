  ...
  public:
   ...
   void Logv(const char* format, va_list arguments)
   {
      ...
      for (int iteration = 0; iteration < 2; ++iteration) {
         ...
         int buffer_offset = snprintf(buffer, buffer_size, "[%04d-%02d-%02d %02d:%02d:%02d.%06d][%s] ", now_components.tm_year + 1900,
                                      now_components.tm_mon + 1, now_components.tm_mday, now_components.tm_hour, now_components.tm_min,
                                      now_components.tm_sec, static_cast<int>(now_timeval.tv_usec), thread_id.substr(thread_id.size() - 5).c_str());
