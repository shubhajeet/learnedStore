...
LeanStore::LeanStore()
{
   ...
   std::cout << "ssd: " << FLAGS_ssd_path << " ssd_fd: " << ssd_fd << std::endl;
   ...
   if (FLAGS_recover) {
      ...
      std::cout << "Loading of meta data took " << duration / 1000000 << "ms" << std::endl;
   }
   // -------------------------------------------------------------------------------------
...
void LeanStore::deserializeState()
{
   ...
   for (auto& dt : dts.GetArray()) {
      ...
      if (dt_type == 0) {
         ...
         // Probably should start auto training thread here
