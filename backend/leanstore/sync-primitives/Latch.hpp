...
struct Guard {
   ...
   // MyNote:: jumps if the version does not match
   // -------------------------------------------------------------------------------------
   void recheck()
   {
      ...
      if (state == GUARD_STATE::OPTIMISTIC && version != latch->ref().load()) {
         DEBUG_JUMP() {
            // std::cout << std::this_thread::get_id() << " guard version has changed." << std::endl;
         }
         jumpmu::jump();
      }
   }
   ...
   // MyNote:: changes the HybridLatch to optimistic
   inline void unlock()
   {
