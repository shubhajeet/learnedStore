...
{
// -------------------------------------------------------------------------------------
u8 BTreeNode::getStringHash(const u8* key_bytes, u16 len)
{
   // return key_bytes[0];
   hash<std::string> hasher;
   return 0XFF && hasher(std::string((char*)key_bytes, len));
}

u8 BTreeNode::getKeyHash(u16 slotId)
{
   auto len = getKeyLen(slotId);
   u8 key_bytes[len + 1];
   copyKeyWithoutPrefix(slotId, key_bytes);
   key_bytes[len] = 0;
   return getStringHash(key_bytes, len);
}
...
//    hint[i] = slot[dist * (i + 1)].head;
   ...
   if (is_leaf && count < 64) {
      ...
      DEBUG_BLOCK()
      {
         ...
         // if (unique != count)
         // {
         //    std::cout << "Unique: " << unique << " out of " << count << std::endl;
         // }
      }
      return;
   }
   ...
   u16 dist = count / (hint_count + 1);
   dist = (dist == 0) ? 1 : dist;
   for (u16 i = 0; i < hint_count && (dist * (i + 1)) < count; i++)
      hint[i] = slot[dist * (i + 1)].head;
}
// -------------------------------------------------------------------------------------
...
#endif
   u16 dist = count / (hint_count + 1);
   // Remove in case optimization is not possible
   dist = (dist == 0) ? 1 : dist;
   u16 begin = 0;
...
#ifdef COUNT_LEAF_AND_KEY
   leaf_count++;
   std::cout << "[leaf_key_stats] " << leaf_count << " " << key_count << std::endl;
#endif
   // -------------------------------------------------------------------------------------
   // My note: Not sure why we want the keys with max common prefix in the same node
...
s32 BTreeNode::compareKeyWithBoundaries(const u8* key, u16 keyLength)
{
   ...
   // auto key_int = utils::u8_to<KEY>(key, keyLength);
