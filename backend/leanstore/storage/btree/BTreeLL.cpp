...
// OP_RESULT BTreeLL::lookup_rs(u8* key, u16 key_length, function<void(const u8*, u16)> payload_callback)
OP_RESULT BTreeLL::lookup_rs(KEY key, function<void(const u8*, u16)> payload_callback)
{
   ...
   BufferFrame* found = nullptr;
   if (spline_idx == spline_predictor.spline_points_.size()) {
#ifdef COMPACT_MAPPING
      auto info = BMC::global_bf->getPageinBufferPool(mapping_pid[mapping_pid.size() - 1]);
      found = info.bf;
#else
      auto info = BMC::global_bf->getPageinBufferPool(secondary_mapping_pid[secondary_mapping_pid.size() - 1].second);
      found = info.bf;
#endif
   } else {
      found = jumpToLeafUsingSegment(key, spline_idx);
   }
   if (found != nullptr) {
...
// OP_RESULT BTreeLL::fast_trained_lookup_rs(KEY key, function<void(const u8*, u16)> payload_callback)
// {
//    auto key_length = sizeof(KEY);
//    u8 key_bytes[key_length];
//    fold(key_bytes, key);
// #ifdef LATENCY_BREAKDOWN
// #ifdef USE_TSC
//    static auto timer_registry = AvgTSCTimerRegistry<std::string>("learnStore function: fast_trained_lookup_rs ");
// #else
//    static auto timer_registry = AvgTimerRegistry<std::string>("learnStore function: fast_trained_lookup_rs");
// #endif
//    auto spline_search_timer = timer_registry.registerObject("GetSplineSegment", "finding_spline_segments");
//    spline_search_timer->start();
// #endif
//    // search for the segment
//    auto spline_idx = spline_predictor.GetSplineSegment(key);
// #ifdef LATENCY_BREAKDOWN
//    spline_search_timer->stop();
// #endif
//    // load the leaf and return
//    BufferFrame* found = nullptr;
//    if (spline_idx == 0) {
...
//       found = mapping_bfs[0];
// #else
//       found = BMC::global_bf->getPageinBufferPool(mapping_pid[0]);
// #endif
// #else
// #ifdef DONT_USE_PID
//       found = secondary_mapping_bf[0].second;
// #else
//       found = BMC::global_bf->getPageinBufferPool(secondary_mapping_pid[0].second);
// #endif
// #endif
//    } else if (spline_idx == spline_predictor.spline_points_.size()) {
// #ifdef COMPACT_MAPPING
// #ifdef DONT_USE_PID
//       found = mapping_bfs[mapping_bfs.size() - 1];
// #else
//       found = BMC::global_bf->getPageinBufferPool(mapping_pid[mapping_pid.size() - 1]);
// #endif
// #else
// #ifdef DONT_USE_PID
//       found = secondary_mapping_bf[secondary_mapping_bf.size() - 1].second;
// #else
//       found = BMC::global_bf->getPageinBufferPool(secondary_mapping_pid[secondary_mapping_pid.size() - 1].second);
// #endif
// #endif
//    } else {
//       found = fastTrainedJumpToLeafUsingSegment(key, spline_idx);
//    }
...
//             return OP_RESULT::NOT_FOUND;
//          }
//          // std::cout << "[fast_trained_lookup_rs pos=-1] spline_index=" << spline_idx << "; key=" << key << "spline_points_[" << spline_idx << "]
//          is
//          // "
//          //           << spline_predictor.spline_points_[spline_idx].x << std::endl;

//          return OP_RESULT::NOT_FOUND;
//       }
//    } else {
//       std::cout << "[fast_trained_lookup_rs null] spline_index=" << spline_idx << "; key=" << key << std::endl;
//       return OP_RESULT::NOT_FOUND;
//    }
// }
...
OP_RESULT BTreeLL::lookup_lr(const KEY key, function<void(const u8*, u16)> payload_callback)
{
   // std::cout << " key: " << key << std::endl;
   auto predict = model.predict(key);
   // auto predict = meta_node_bf->header.model.search(key);
   // std::cout << "predict: " << predict << std::endl;
   // auto error = meta_node_bf->header.model.get_error();
// binary_search in mapping keys using predict and error
#ifdef COMPACT_MAPPING
   auto key_size = mapping_key.size();
#else
#ifdef DONT_USE_PID
   auto key_size = secondary_mapping_bf.size() - 1;
#else
   auto key_size = secondary_mapping_pid.size() - 1;
#endif
#endif
   // binary_search in mapping keys using predict and error
#ifdef COMPACT_MAPPING
// auto it = std::lower_bound(mapping_key.begin() + bound.begin, mapping_key.begin() + bound.end, key, [](const KEY a, KEY val) { return a < val;
// });

// if (it == mapping_key.end()) {
//    // raise(SIGTRAP);
//    return OP_RESULT::NOT_FOUND;
// }
// auto lpos = std::distance(mapping_key.begin(), it);
#ifdef LR_EXPONENTIAL_SEARCH
   auto lpos = model.exponential_search(mapping_key, key, predict);
#else
   auto bound = model.get_searchbound(predict, key_size);
   auto lpos = model.binary_search(mapping_key, key, bound.begin, bound.end);
#endif
   // if (lpos >= mapping_bfs.size()) {
   //    std::cout << "lpos: " << lpos << " predict: " << predict << " key_size: " << key_size << std::endl;
   // }
   lpos = (lpos >= key_size) ? key_size - 1 : lpos;
   // ensure(lpos == predict);
   // if (lpos != predict) {
   //    std::cout << "lpos: " << lpos << " predict: " << predict << " key_size: " << key_size << std::endl;
   // }
   // std::cout << "lpos: " << lpos << " predict: " << predict << " key_size: " << key_size << std::endl;
   // ensure(lpos == round(predict));
   // INFO("key: %lu lpos: %lu predict: %lu", key, lpos, predict);

// lpos = (lpos != 0) ? lpos - 1 : 0;
// ensure(mapping_key[lpos - 1] <= key);
#ifdef DONT_USE_PID
   auto lbf = mapping_bfs[lpos];
#ifdef PID_CHECK
   auto lpid = mapping_pid[lpos];
   if (lbf == nullptr || lbf->header.pid != lpid) {
...
#else
#ifndef DONT_USE_PID
   auto it = std::lower_bound(secondary_mapping_pid.begin() + lower_bound, secondary_mapping_pid.begin() + upper_bound, key,
                              [](const std::pair<KEY, PID>& a, const KEY val) { return a.first < val; });
   // auto lpos = std::distance(secondary_mapping_pid.begin(), it);
   // lpos = (lpos != 0) ? lpos - 1 : 0;
   // auto lpid = secondary_mapping_pid[lpos].second;
   auto lpid = it->second;
#else
   auto it = std::lower_bound(secondary_mapping_bf.begin() + lower_bound, secondary_mapping_bf.begin() + upper_bound, key,
                              [](const std::pair<KEY, BufferFrame*>& a, const KEY val) { return a.first < val; });
   auto lpos = std::distance(secondary_mapping_bf.begin(), it);
   lpos = (lpos >= key_size) ? key_size - 1 : lpos;
   auto lbf = secondary_mapping_bf[lpos].second;
#ifdef PID_CHECK
   auto lpid = secondary_mapping_pid[lpos].second;
   // INFO("key: %lu lpos: %lu pid_size: %lu lbf: %lu lpid: %lu", key, lpos, secondary_mapping_pid.size(), lbf, lpid);
   // auto lbf = it->second;
   // INFO("Check lbf->header.pid: %lu lpid: %lu lbf: %lu", lbf->header.pid, lpid, lbf);
   if (lbf == nullptr || lbf->header.pid != lpid) {
      // INFO("Unequal lbf->header.pid: %lu lpid: %lu", lbf->header.pid, lpid);
      lbf = BMC::global_bf->getPageinBufferPool(lpid);
   }
#endif
#endif
#endif
   ...
   auto bf = lbf;
#else
   auto bf = BMC::global_bf->pageInBufferFrame(lpid);
   if (bf == nullptr) {
      bf = BMC::global_bf->getPageinBufferPool(lpid);
   }
#endif
   if (bf == nullptr)
      return OP_RESULT::NOT_FOUND;
   HybridPageGuard<BTreeNode> leaf(bf);
   s16 pos = 0;
#ifdef MODEL_IN_LEAF_NODE
#ifdef MODEL_LR
   auto swip = leaf.swip();
   bf = swip.bfPtr();
   predict = bf->header.model.predict(key);
   auto key_length = sizeof(KEY);
   u8 key_bytes[key_length];
   fold(key_bytes, key);
#ifdef EXPONENTIAL_SEARCH
   ...
   bool isequal = false;
   auto search_bound = bf->header.model.get_searchbound(predict, leaf->count);
   pos = leaf->binarySearch(key_bytes, key_length, search_bound.begin, search_bound.end, &isequal);
#endif
#else
   auto spline_idx = leaf->header.splines.GetSplineSegment(key);
   auto predict = leaf->header.splines.GetEstimatedPosistion(key, spline_idx);
   auto search_bound = leaf->header.splines.GetSearchBound(predict);
   pos = leaf->binary_search(key_bytes, key_length, search_bound.begin, search_bound.end);
#endif
#else
   ...
   u8 key_bytes[key_length];
   fold(key_bytes, key);
   // ensure(leaf.bf->header.pid == lpid);
   DEBUG_BLOCK()
   {
      s16 sanity_check_result = leaf->compareKeyWithBoundaries(key_bytes, key_length);
      leaf.recheck();
...
OP_RESULT BTreeLL::lookup_lr(u8* key, u16 key_length, function<void(const u8*, u16)> payload_callback)
{
   auto key_int = utils::u8_to<KEY>(key, key_length);
   auto predict = model.predict(key_int);
   // std::cout << "predict: " << predict << std::endl;
   // auto error = model.get_error();
// binary_search in mapping keys using predict and error
#ifdef COMPACT_MAPPING
   auto key_size = mapping_key.size();
#else
   auto key_size = secondary_mapping_pid.size();
#endif
   auto bound = model.get_searchbound(predict, key_size);
#ifdef COMPACT_MAPPING
   // auto it =
   //     std::lower_bound(mapping_key.begin() + bound.begin, mapping_key.begin() + bound.end, key_int, [](const KEY a, KEY val) { return a <= val;
   //     });
   // auto lpos = std::distance(mapping_key.begin(), it);
   auto lpos = model.binary_search(mapping_key, key_int, bound.begin, bound.end);
   lpos = (lpos != 0) ? lpos - 1 : 0;
   auto lpid = mapping_pid[lpos];
#else
   auto it = std::lower_bound(secondary_mapping_pid.begin() + lower_bound, secondary_mapping_pid.begin() + upper_bound, key_int,
                              [](const std::pair<KEY, PID>& a, const KEY val) { return a.first <= val; });
   auto lpos = std::distance(secondary_mapping_pid.begin(), it);
   lpos = (lpos != 0) ? lpos - 1 : 0;
   auto lpid = secondary_mapping_pid[lpos].second;
#endif
   auto info = BMC::global_bf->pageInBufferFrame(lpid);
   if (info.bf == nullptr) {
      info = BMC::global_bf->getPageinBufferPool(lpid);
   }
   HybridPageGuard<BTreeNode> leaf(info.bf);
   info.lower_fence = utils::u8_to<KEY>(leaf->getLowerFenceKey(), leaf->lower_fence.length);
   info.upper_fence = utils::u8_to<KEY>(leaf->getUpperFenceKey(), leaf->upper_fence.length);
   // ensure(leaf.bf->header.pid == lpid);
   DEBUG_BLOCK()
   {
      s16 sanity_check_result = leaf->compareKeyWithBoundaries(key, key_length);
...
OP_RESULT BTreeLL::fast_trained_lookup_rmi(const KEY key)
{
   // Just to test the effect of function call
   // size_t leaf_idx = rmi_predictor.get_index(key);
   // return (mapping_key[leaf_idx] == key) ? OP_RESULT::OK : OP_RESULT::NOT_FOUND;
   // if (trained && rmi_predictor.isWithinKeySpace(key)) {
   if (std::shared_lock<std::shared_mutex> lock(model_lock); lock.owns_lock() && trained && rmi_predictor.isWithinKeySpace(key)) {
      size_t leaf_idx = rmi_predictor.get_index(key);
      // return (mapping_key[leaf_idx] == key) ? OP_RESULT::OK : OP_RESULT::NOT_FOUND;
      BufferFrame* leaf_bf = mapping_bfs[leaf_idx];
#ifdef PID_CHECK
...
//          }
   ...
   // INFO("2 leaf_bf: %p key: %lu", leaf_bf, key);
...
OP_RESULT BTreeLL::fast_trained_lookup_rmi(const KEY key, function<void(const u8*, u16)> payload_callback)
{
   // Just to test the effect of function call
   // size_t leaf_idx = rmi_predictor.get_index(key);
   // return (mapping_key[leaf_idx] == key) ? OP_RESULT::OK : OP_RESULT::NOT_FOUND;
...
OP_RESULT BTreeLL::fast_trained_lookup_rs(const KEY key)
{
   // Just to test the effect of the function call
   // auto key_idx = rs_spline_predictor.find(key, mapping_key);
   // return (mapping_key[key_idx] == key) ? OP_RESULT::OK : OP_RESULT::NOT_FOUND;

   // if (trained && rs_spline_predictor.isWithinKeySpace(key)) {
   if (std::shared_lock<std::shared_mutex> lock(model_lock); lock.owns_lock() && trained && rs_spline_predictor.isWithinKeySpace(key)) {
      auto key_idx = rs_spline_predictor.find(key, mapping_key);
      auto leaf_bf = mapping_bfs[key_idx];

#ifdef PID_CHECK
      if (auto pid = mapping_pid[key_idx]; leaf_bf == nullptr || leaf_bf->header.pid != pid) {
         auto info = BMC::global_bf->pageInBufferFrame(pid);
         leaf_bf = info.bf;
         mapping_bfs[key_idx] = leaf_bf;
      }
#endif
      if (leaf_bf != nullptr) {
         HybridPageGuard<BTreeNode> leaf(leaf_bf);
...
OP_RESULT BTreeLL::fast_trained_lookup_rs(const KEY key, function<void(const u8*, u16)> payload_callback)
{
   // Just to test the effect of the function call
   // auto key_idx = rs_spline_predictor.find(key, mapping_key);
   // return (mapping_key[key_idx] == key) ? OP_RESULT::OK : OP_RESULT::NOT_FOUND;
...
void BTreeLL::train_lr()
{
   // scanAll();
   // std::cout << "Single cycle through leaf nodes" << std::endl;
   // return;
#ifdef COMPACT_MAPPING
   mapping_key.clear();
   mapping_pid.clear();
   ...
   leaf_count = pids.size();
   // std::cout << " key count: " << keys.size() << " pid count: " << pids.size() << " bf count: " << bfs.size() << std::endl;
   INFO("key count: %lu pid count: %lu bf count: %lu", keys.size(), pids.size(), bfs.size());
   for (auto i = 0; i < keys.size(); i++) {
      HybridPageGuard<BTreeNode*> guard(bfs[i]);
#ifdef COMPACT_MAPPING
      mapping_key.push_back(keys[i]);
      mapping_pid.push_back(pids[i]);
      mapping_bfs.push_back(bfs[i]);
#else
      secondary_mapping_pid.push_back(make_pair(keys[i], pids[i]));
      secondary_mapping_bf.push_back(make_pair(keys[i], bfs[i]));
#endif
   }
#ifdef COMPACT_MAPPING
   // mapping_key.push_back(std::numeric_limits<KEY>::max());
   mapping_pid.push_back(pids[leaf_count]);
   mapping_bfs.push_back(bfs[leaf_count]);
#else
   secondary_mapping_pid.push_back(make_pair(std::numeric_limits<KEY>::max(), pids[leaf_count]));
   secondary_mapping_bf.push_back(make_pair(std::numeric_limits<KEY>::max(), bfs[leaf_count]));
#endif
   // meta_node_bf->header.model.train(keys);
   model.train(keys);
   // DEBUG_BLOCK()
   {
      std::cout << "leaf count: " << leaf_count << std::endl;
      std::cout << "total kv: " << total_kv << std::endl;
      std::cout << "model line: " << model.m << " x + " << model.c << std::endl;
      std::cout << "model error: " << model.error << std::endl;
   }
...
#endif
#ifdef MODEL_RS
   auto rsb = rsindex::Builder<KEY>(keys.front(), keys.back(), max_error);
   for (auto i = 0; i < keys.size(); i++) {
      auto key = keys[i];
      rsb.AddKey(key);
   }
   rs_spline_predictor = rsb.Finalize();
#endif

#ifdef MODEL_RMI
   rmi_predictor.init(keys, bfs);
   rmi_predictor.train_rmi(FLAGS_nmodels);
#endif
